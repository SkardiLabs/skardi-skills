...
def overview(con):
    r = con.execute(
        ...
        " COUNT(DISTINCT session_id) sess FROM query_audit"
    ).fetchone()
    if not r["n"]:
        print("Database is empty. skardi-server must run with --query-audit-db, and only ad-hoc /query statements are recorded.")
        return
    print(f"{r['n']} rows | {r['ok']} succeeded, {r['bad']} failed | {r['sess'] or 0} sessions")
    ...
    for s in con.execute(
        ...
        " FROM query_audit GROUP BY session_id ORDER BY lo"
    ):
        bad = f", {s['bad']} failed" if s["bad"] else ""
        print(f"  {s['s']:24} {s['n']:>3} rows{bad}")


def rows(con, limit, session, failed_only, full):
...
def main():
    ...
    if a.overview:
        ...
        rows(con, a.limit, a.session, a.failed, a.full_sql)
