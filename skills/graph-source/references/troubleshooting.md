...
| `[42703]: could not find rte for <alias>` from a Cypher query | AGE cannot `ORDER BY` an alias — in a `RETURN` **or** a `WITH`; measured, both fail identically, so a `WITH n.name AS p ORDER BY p` matches this row too. Order by the original expression (`ORDER BY p.age`, `ORDER BY count(*) DESC`) or sort on the SQL side. An aggregate expression is fine; it is the name that cannot be resolved. |
...
| `graph name is invalid` from `create_graph` | The name broke one of three rules, and the message names none of them. Measured: shorter than three characters (`kg`, `ab`, `zz` all refused), starts with a digit (`1ab`, `12345`), or contains a space (`a b`). Uppercase, `_` and `-` are fine once the name is long enough. The three-character floor is the one that bites, because `kg` is the natural pick — but that is Skardi's CATALOG name, a different namespace, and it never needed to match the AGE graph. |
| `syntax error at or near "-"` inside a `cypher(…, $$ … $$)` block | A `--` comment. That is SQL's marker and the Cypher parser refuses it; inside the dollar quotes the comment marker is `//`. `--` outside them is still fine. |
