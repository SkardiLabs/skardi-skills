...
- **Source strings must be unique across the whole workspace, not just within one run.** The manifest and the index are shared by every entry, so a table row whose source equals a file's relative path is the *same document id* as that file. Both scripts now record which entry each source came from and **refuse to start** when a source is already held by a different one, naming the collisions — because the two ways it used to fail were both quiet: identical content was written off as `already ok`, and differing content was reported as a "changed file" that then never got indexed. If you hit the refusal, give the run a distinct `--label`, point `--source-column` at real locators, or use a separate workspace.
