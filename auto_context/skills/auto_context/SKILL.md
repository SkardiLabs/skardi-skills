...
- **Source strings must be unique across the whole workspace, not just within one run.** The manifest and the index are shared by every entry, so a table row whose source equals a file's relative path is the *same document id* as that file. Both scripts record which entry each source came from and **refuse to start** when a source is already held by the other one, naming the collisions — because the two ways it used to fail were both quiet: identical content was written off as `already ok`, and differing content was reported as a "changed file" that then never got indexed. If you hit the refusal, make the two sets of strings distinct (`--source-column` at real locators, or a `--label` that cannot collide with corpus paths) or render a separate workspace. What is recorded is the entry, not its arguments, so re-running the same table under a different `--label` is not a collision.
...
- **Empty-content rows are a signal, not noise.** On a table landed by the fetch-and-land process they are documents that were listed but never fetched. Pass **`--require-complete`** there and the run refuses to index a partial corpus instead of merely mentioning it; without the flag it warns and continues, which is fine for an ordinary table but not for a fetched one. See [references/fetch_and_land.md](references/fetch_and_land.md) for what this does and does not prove.
...
- Never report a `--limit` run as a finished ingest; it holds rows back by construction and says so.
