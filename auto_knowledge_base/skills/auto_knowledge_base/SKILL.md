...
- **Different chunk size / overlap.** Pass `--chunk-size 800 --overlap 100` (or whatever) to `ingest_corpus.py`. The constraint is `overlap < chunk_size`; both must be positive integers. There's no "right" answer — 1200/200 (the default) suits most prose; tighter chunks (500/50) help when the corpus has dense factual passages and queries target single sentences. **Mind the model's token cap:** `--chunk-size` counts *characters*, but bge/e5/BERT models max out at 512 *tokens* and candle does not auto-truncate — an over-cap chunk fails at INSERT with `invalid index 512 with dim size 512` and that file silently drops. Prose (~4 chars/token) is safe at 1200; **code is ~2.5–3 chars/token, so use `--chunk-size 800 --overlap 120` for code corpora** (or a long-context model like `nomic-embed-text`, 8k). See [references/troubleshooting.md](references/troubleshooting.md).
