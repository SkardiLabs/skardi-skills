...
- 任何取数失败(节点/分页/取正文)默认使整次同步失败、不覆盖旧表;要"能拿多少算多少"须显式 --allow-partial。
目标 lark-cli 版本 = 最新(用户 npm 装到的即最新):docs +fetch 用 --doc-format markdown、正文在 data.document.content。"""
import argparse, json, subprocess, sqlite3, sys, re, os
from datetime import datetime

def lark(args):
    """跑 lark-cli --as user；失败即报错(docs/wiki/api 默认吐 JSON,无需 --json)。"""
...
def lark_try(args):
    """非致命版:失败返回 None，用于版本探测回退 / 允许跳过。"""
    p = subprocess.run(["lark-cli", *args, "--as", "user"], capture_output=True, text=True)
    out = "".join(l for l in p.stdout.splitlines() if not l.startswith("[page"))
    if p.returncode != 0:
        return None
    try:
...
def resolve_doc(token_or_url):
    ...
    if "/wiki/" in token_or_url:            # wiki 节点要解析到底层 docx obj_token
        d = lark_try(["wiki", "spaces", "get_node", "--params", json.dumps({"token": tok})])
        node = (d or {}).get("node")
        if node and node.get("obj_type") in ("docx", "doc") and node.get("obj_token"):
            return node["obj_token"], node.get("title") or tok
    return tok, None                         # 当作已是 docx token(取正文阶段再判失败)

def list_children(space, node, errors):
    items, page = [], None
    while True:                                  # 翻页,避免子节点 >50 被静默截断
        ...
        if d is None:                            # 某页失败:记错,不再往下(交由 main 决定是否整体失败)
            errors.append(f"list_children failed under node {node}")
            break
...
def collect_from_node(space, node_token, acc, errors):
    """递归收集 wiki 子树里所有 docx 节点 → [(doc_id, title, wiki_ref)]；单节点失败记入 errors。"""
    if not node_token:
        return
    d = lark_try(["wiki", "spaces", "get_node", "--params", json.dumps({"token": node_token})])
    if d is None:
        errors.append(f"get_node failed: {node_token}")
        return
    node = d.get("node") or {}
    if node.get("obj_type") in ("docx", "doc") and node.get("obj_token"):
        acc.append((node["obj_token"], node.get("title"), f"wiki/{node_token}"))
    if node.get("has_child"):
        for ch in list_children(space, node_token, errors):
            collect_from_node(space, ch.get("node_token"), acc, errors)

def fetch_content(doc_id):
    """取整篇正文 markdown(1.0.68:--doc-format markdown,正文在 data.document.content)。"""
    d = lark(["docs", "+fetch", "--doc", doc_id, "--doc-format", "markdown"])
    return (d.get("document") or {}).get("content", "")

def fetch_title(doc_id):
    """标题走 docx 元数据 API(fetch 的 title 字段在 1.0.68 已消失)。"""
...
def main():
    ...
    ap.add_argument("--allow-partial", action="store_true",
                    help="sync whatever succeeded even if some docs/nodes failed (default: any failure aborts without touching the old table)")
    a = ap.parse_args()
    ...
    errors = []
    # 1) 汇总要同步的文档清单 (doc_id, wiki_title, source_ref)
    targets = []
    for d in a.doc:
        doc_id, title = resolve_doc(d)
        targets.append((doc_id, title, d))
    if a.node:
        collect_from_node(a.space, a.node, targets, errors)
    seen, uniq = set(), []
    for t in targets:
        if t[0] not in seen:
    ...
    for doc_id, wiki_title, url in uniq:
        try:
            ...
            errors.append(f"fetch failed: {doc_id} ({e})")
            continue
        title = wiki_title or fetch_title(doc_id) or doc_id
        rows.append((doc_id, title, url, md, now))

    # 3) 失败门禁:默认任何失败都不覆盖旧表(避免"静默部分成功"看起来像完成)
    if errors and not a.allow_partial:
        detail = "\n".join("  - " + e for e in errors[:20])
        more = "" if len(errors) <= 20 else f"\n  …(+{len(errors)-20} more)"
        sys.exit(f"sync had {len(errors)} failure(s); NOT replacing existing data. "
                 f"Re-run with --allow-partial to keep what succeeded.\n{detail}{more}")

    # 4) 原子替换:写临时表 → 成功后丢旧表、改名(失败则旧表完好保留)
    tmp = tbl + "__new"
    ...
    try:
        os.chmod(a.out, 0o600)   # 本地快照含文档正文:收紧为仅当前用户可读
    except OSError:
        pass

    print(f"OK: synced {len(rows)} docs" + (f" ({len(errors)} failed, --allow-partial)" if errors else "") +
          f" -> {a.out} (table '{tbl}')")
    for r in rows:
