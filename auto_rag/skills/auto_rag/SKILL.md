...
- **Different chunk size / overlap.** Pass `--chunk-size` / `--overlap` to `ingest_corpus.py` (per-corpus knobs) — they're pipeline parameters, not template-time substitutions, so changing them needs no re-render. The constraint is `overlap < chunk_size`; both must be positive integers. **`--chunk-size` is in characters, but the embedding model has a *token* cap** (512 for the bge/e5/BERT families) and candle does not auto-truncate — a chunk over the cap fails with `invalid index 512 with dim size 512` and that file drops out. Prose at ~4 chars/token is safe at the 1200 default; **code is ~2.5–3 chars/token, so for a code corpus drop to `--chunk-size 800 --overlap 120`** (or use a long-context model like `nomic-embed-text`). See [references/troubleshooting.md](references/troubleshooting.md) § Embedding-specific.
