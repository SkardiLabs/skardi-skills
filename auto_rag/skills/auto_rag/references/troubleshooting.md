...
| `Embedding failed: Model forward pass failed: index-select invalid index 512 with dim size 512` (or any `invalid index N with dim size N`) on some — not all — files | A chunk tokenizes to more tokens than the embedding model's max sequence length (512 for the BERT/bge/e5/DistilBERT families). candle does **not** auto-truncate. `--chunk-size` is measured in **characters, not tokens**, and dense text — especially **code** (~2.5–3 chars/token vs ~4 for prose) — can blow past the cap even at the 1200-char default. The nasty part: only the *largest* source files fail, so they silently drop out of the corpus while ingest reports mostly-success. | Lower `--chunk-size` on `ingest_corpus.py` so the worst-case chunk stays under the model's token cap. For a 512-token model over a **code** corpus, `--chunk-size 800 --overlap 120` is a safe starting point; go lower for minified/single-line files. Alternatively switch to a long-context embedding model (e.g. `nomic-embed-text`, 8k). Re-ingest the affected files after lowering (they're marked `err:` in `ingest_progress.json`, so a rerun retries exactly them). |
