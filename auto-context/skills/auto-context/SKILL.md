...
**Decision rule.** Local-only, or is a hosted API acceptable? If local, choose candle vs gguf on memory budget. Code corpus → `voyage-code-3` or `jina-embeddings-v2-base-code`. Multilingual → `multilingual-e5-large` or `text-embedding-3-large`. Chunks over 512 tokens → `nomic-embed-text`. Otherwise bge on candle. **A 512-token cap interacts with `--chunk-size`, which is measured in characters** — see the note under Step 3 before indexing a code or CJK corpus. For CJK the model choice is not the fix: every listed model still caps at 512 tokens, and CJK runs near 1 char/token, so `--chunk-size` has to come down regardless.
